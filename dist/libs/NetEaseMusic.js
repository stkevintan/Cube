...
function transfer(results) {
    ...
    process.nextTick(function() {
        ...
        for (var k = 0; k < num; k++) {
            var idTmp = idArray.slice(k * 100, Math.min((k + 1) * 100,
                idArray.length));
            nem.songsDetail(idTmp, function(err, res) {
                if (err) {
                    console.warn('Failed to Fetch Song Details',
                        err.stack);
                    return;
                }
...
var nem = {
    login: function(username, password, callback) {
        ...
        httpRequest('post', url, encBody, function(err, res) {
            ...
            if (!doc.profile) callback(new Error(
    ...
    userProfile: function(uid, callback) {
        uid = uid || getUserID();
        if (!uid) {
            callback(new Error('User Hasnt Logged In'));
            return;
        }
        ...
        httpRequest('get', url, {
            ...
            var doc = res.body;
            if (doc.code != 200) callback(new Error(
                'Http Code:' + doc.code));
