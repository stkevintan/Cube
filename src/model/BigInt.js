 ...
 */
module.exports = {
    newInstance: function (src, radix) {
        //copy from other
        if (!src)return new BigInt();
        if (isInstance(src)) return radix == 16 ? new BarrettMu(src) : biCopy(src);
        if (typeof src === 'number') return biFromNumber(src);
        radix = Number(radix);
        if (isNaN(radix))throw 'radix must be integer';
        radix = radix || 10;
        return radix == 16 ? biFromHex(src) : biFromString(src, radix);
    },
...
BigInt.prototype.toString = function (encoding) {
    if (encoding == 'hex') return biToHex(this, 10);
    else return biToString(this);
}
